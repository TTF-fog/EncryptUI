...
func makeNewStandalone(s string, executeMode bool, directory string, password string, remove_workspace bool) {
	var b bytes.Buffer
	standalone = "//THIS IS AN AUTOGENERATED FILE, DO NOT MODIFY BY EncryptUI, IGNORE THE NEXT LINE" + standalone
	w := compress.NewWriter(&b)
	w.Write([]byte(s))
	w.Close()
	compressed := b.Bytes()
	encrypted, _ := EncryptAES([]byte(password), compressed)
	os.WriteFile(directory+"/encrypt.txt", []byte(encrypted), 777)
	os.WriteFile(directory+"/standalone.go", []byte(standalone), 777)
	if remove_workspace {
		os.Remove(directory)
	}
